		...
		GUILayout.BeginArea (new Rect (20, 20, Screen.width - 40, 450));
			scrollPosition=GUILayout.BeginScrollView(scrollPosition);
				GUILayout.BeginHorizontal();
					...
					if (b_isEngineShop)
					{
						m_engineControl = RocketList.GetInstance().GetEnginePrefab(name).GetComponent<EngineControl>();
					}
					else
					{
						m_rocketControl = RocketList.GetInstance().GetRocketPrefab(name).GetComponent<RocketControl>();
					}
						GUILayout.BeginVertical();
							...
							if (b_isEngineShop)
							{
								if (GameProgress.buyedEngines.ContainsKey(name))
								{
									...
									{
										GameObject newEngine = RocketList.GetInstance().GetEnginePrefab(name);
							...
							else
							{
								if (GameProgress.buyedRockets.ContainsKey(name))
								{
									if (GUILayout.Button(Localization.T_SELECT))
									{
										GameObject newRocket = RocketList.GetInstance().GetRocketPrefab(name);
