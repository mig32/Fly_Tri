...
public class Menu : MonoBehaviour {
	...
	void OnGUI() {
		...
		else if (b_isShowMenu) 
		{
			...
			if (WorldControl.GetInstance() != null)
			{
				WorldControl.GetInstance().SetMusicVolume(GUILayout.HorizontalSlider(WorldControl.GetInstance().getSoundVolume(), WorldControl.MIN_SOUND_VOLUME, WorldControl.MAX_SOUND_VOLUME));
			}
			GUILayout.Space(10);
			GUILayout.Label(Localization.T_FX_VOLUME);
			if (WorldControl.GetInstance() != null)
			{
				WorldControl.GetInstance().SetFXVolume(GUILayout.HorizontalSlider(WorldControl.GetInstance().m_fxVolume, WorldControl.MIN_SOUND_VOLUME, WorldControl.MAX_SOUND_VOLUME));
			}
			GUILayout.FlexibleSpace ();
			if (b_isStartMenuButtons) 
